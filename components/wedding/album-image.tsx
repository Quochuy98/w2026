...
export function AlbumImageView({ image, priority = false, sizes = "100vw", className = "", onClick }: AlbumImageViewProps) {
  ...
  const isImageKit = !isLocal && Boolean(imageKitEndpoint && image.path);
  const content = failed ? (
    <span className="flex h-full min-h-40 items-center justify-center px-6 text-center text-sm text-[var(--muted)]">Ảnh tạm thời chưa thể tải.</span>
  ) : (
    isImageKit ? (
      <ImageKitImage
        ...
        priority={priority}
        responsive
        transformation={[{ width: 1440, quality: 82, format: "webp" }]}
