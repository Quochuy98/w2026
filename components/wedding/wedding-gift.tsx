...
import type { BankAccount } from "@/content/wedding";

function GiftBox({ gift }: { gift: BankAccount }) {
...
export function WeddingGift() {
  ...
  return (
    <section id="gifts" aria-labelledby="gifts-heading" className="relative mx-auto w-full max-w-[1400px] px-5 py-20 sm:px-8 sm:py-28 lg:px-12">
      ...
      <div className="mx-auto grid max-w-xl gap-6 lg:max-w-3xl lg:grid-cols-2">
        {gifts.map((gift) => (
          <GiftBox key={gift.accountNumber} gift={gift} />
        ))}
      </div>
    </section>
  );
